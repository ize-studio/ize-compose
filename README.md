...
Firmware v1.3.x supports browser-based manual firmware update by selecting a `.bin` file on the WebServer page and uploading it to the device. Starting with v1.4.0, Ize Compose can automatically handle the full Online Update flow from GitHub Release asset download through firmware update. v1.4.4 adds automatic document saving and automatic GitHub Release checks after Wi-Fi/network activity; it keeps the v1.4.2 SD browser page at `/ize_compose/ize_compose_1-4-2.html`. Use `autoupdate/izefirmware.bin` or `autoupdate/izefirmware_v1.4.4.bin` as the firmware file.
...
v1.4.0 adds the top-level `Sync` flow for GitHub private repository sync, then turns the Wi-Fi adapter off after sync. The Bluetooth feature that acted as an external keyboard in v1.3.0 was removed because of battery-use issues. See [RELEASE_1.4.4.md](RELEASE_1.4.4.md) for the current release notes, [RELEASE_1.4.3.md](RELEASE_1.4.3.md) for the previous sync fix, and [RELEASE_1.4.0.md](RELEASE_1.4.0.md) for the original v1.4.0 release.
...
- v1.4.4 saves the current document before opening another document, before creating a new document, before sleep, and every 3 minutes while editing
...
- v1.4.4 checks GitHub Release status automatically after Wi-Fi connects and after GitHub document sync, so update notices can appear on the device menu without a separate browser-side release check.
...
Ize-compose/
  ...
  RELEASE_1.4.4.md   - v1.4.4 release notes
