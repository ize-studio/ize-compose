...
For `v1.4.0-test`, GitHub sync is experimental. The browser setup page can save repository settings, but the actual top-level `Sync` path still needs real-device verification.
...
## v1.4.0-test Unified Page

`/ize_compose/ize_compose_1-4-0-test.html` is now the single browser page for both WiFi mode and WebServer mode. It has two tabs:
